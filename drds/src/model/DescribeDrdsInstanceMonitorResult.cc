...
DescribeDrdsInstanceMonitorResult::DescribeDrdsInstanceMonitorResult() :
	ServiceResult()
{}

DescribeDrdsInstanceMonitorResult::DescribeDrdsInstanceMonitorResult(const std::string &payload) :
	ServiceResult()
{
	parse(payload);
}
...
void DescribeDrdsInstanceMonitorResult::parse(const std::string &payload)
{
	...
	for (auto value : allData)
	{
		...
		if(!value["NodeNum"].isNull())
			dataObject.nodeNum = std::stoi(value["NodeNum"].asString());
